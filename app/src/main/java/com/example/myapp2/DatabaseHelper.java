...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    public void addCustomer(Customer customer) {
        ...
        if (newRowId != -1) {
            ...
            Log.d("DatabaseHelper", "Added customer: " + customer.getName() + " with ID: " + newRowId);
        } else {
            Log.e("DatabaseHelper", "Failed to add customer: " + customer.getName());
        }
        db.close();
    }
    ...
    public ArrayList<Customer> getAllCustomers() {
        ...
        Log.d("DatabaseHelper", "Fetching all customers, cursor count: " + cursor.getCount());
        if (cursor.moveToFirst()) {
            do {
                ...
                Log.d("DatabaseHelper", "Retrieved customer: " + customer.getName());
            } while (cursor.moveToNext());
        } else {
            Log.d("DatabaseHelper", "No customers found in database");
        }
        cursor.close();
        return customers;
    }
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                worklogs.add(new Worklog(
                        ...
                        cursor.getLong(cursor.getColumnIndexOrThrow(COLUMN_CLOCK_IN)), // Fixed order
                        clockOut,
                        cursor.getString(cursor.getColumnIndexOrThrow(COLUMN_DESCRIPTION)) // Fixed order
                ));
            } while (cursor.moveToNext());
        }
        cursor.close();
